...
// *         created on 9/13/15
// */
//object BibtexFeatureDomain1 extends CategoricalVectorDomain[String]
//
//class BibtexFeatures(val token: Token) extends BinaryFeatureVectorVariable[String] {
//  def domain = BibtexFeatureDomain1
...
//    val features = token.attr[BibtexFeatures]
//    assert(features != null, "token has no attr BibtexFeatures")
//    //    val features = new BibtexFeatures(token)
...
//    //      token.attr[BibtexFeatures] += "BIN=" + binned
//    //    }
//
//    for (t <- d.tokens) {
//      if (t.nextWindow(10).count(_.string.toLowerCase.matches(".*(ed\\.|editor|eds|editors).*")) > 0) t.attr[BibtexFeatures] += "POSSIBLEEDITOR"
//    }
//
//    for (s <- d.sentences) {
//      for (token <- s.tokens) {
//        for (lexicon <- lexicons(token)) {
//          token.attr[BibtexFeatures] += "LEX=" + lexicon
//        }
//      }
//      cc.factorie.app.chain.Observations.addNeighboringFeatureConjunctions(s.tokens, (t: Token) => t.attr[BibtexFeatures], "^[^@]*$", List(0, 0), List(1), List(2), List(-1), List(-2))
//    }
//  }
//
