...
import edu.umass.cs.iesl.bibie.load.{LoadHier, PreFeatures}
  ...
  def computeDocumentsFeatures(documents: Seq[Document], training: Boolean = false) = {
    if (training) {
      ...
      while (i < documents.length) {
        computeDocumentFeatures(documents(i))
        i += 1
      }
    } else {
      documents.par.foreach { document => computeDocumentFeatures(document) }
    }
  }
